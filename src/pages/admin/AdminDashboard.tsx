...
import { approveEntryRequest, GatepassEntryRequest, getPendingEntryRequests } from '../../utils/gatepass'
...
const AdminDashboard: React.FC = () => {
  ...
  const [pendingEntryRequests, setPendingEntryRequests] = useState<GatepassEntryRequest[]>([])
  const [approvingRequestId, setApprovingRequestId] = useState<string | null>(null)

  const refreshPendingRequests = () => {
    const requests = getPendingEntryRequests().sort((a, b) => b.requestedAt - a.requestedAt)
  ...
  useEffect(() => {
    ...
    const fetchDashboardStats = async () => {
      ...
      try {
        ...
        const attendanceResponse = await indexer
        ...
        const presentToday =
          attendanceResponse.transactions?.filter((txn: { receiver?: string }) => txn.receiver && STUDENT_ROSTER.includes(txn.receiver))
            .length || 0

        // 3. Certificates Issued (Count of ASA creations by Admin with unitName 'CERT')
        const assetsResponse = await indexer.searchForAssets().creator(activeAddress).unit('CERT').do()
        ...
        const rewardResponse = await indexer
          .searchForTransactions()
          .address(activeAddress)
          .addressRole('sender')
          .txType('pay')
  ...
  const handleApproveEntryRequest = (requestId: string) => {
    ...
    const approvedRequest = approveEntryRequest(requestId)
    if (!approvedRequest) {
      enqueueSnackbar('Entry request was not found.', { variant: 'error' })
    } else if (approvedRequest.wasLate) {
      enqueueSnackbar('Entry approved and marked as late return.', { variant: 'warning' })
    } else {
      enqueueSnackbar('Entry approved successfully.', { variant: 'success' })
    }
    refreshPendingRequests()
    ...
    <div className="flex flex-col max-w-5xl mx-auto w-full">
      ...
      {!activeAddress ? (
        <div className="bg-white border rounded-3xl p-12 w-full text-center shadow-lg shadow-teal-100/50 flex flex-col items-center justify-center max-w-2xl mx-auto">
          ...
          <p className="text-teal-600/80 max-w-md font-medium">
            Please connect your authorized administrative wallet to access the command center.
          </p>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Link
            to="/admin/sign"
            className="group bg-white rounded-3xl p-8 shadow-lg shadow-teal-100/50 border border-teal-50 hover:-translate-y-2 hover:shadow-xl hover:shadow-cyan-200/50 transition-all duration-300 relative overflow-hidden flex flex-col"
          >
          ...
          <Link
            to="/admin/mint"
            className="group bg-white rounded-3xl p-8 shadow-lg shadow-teal-100/50 border border-teal-50 hover:-translate-y-2 hover:shadow-xl hover:shadow-fuchsia-200/50 transition-all duration-300 relative overflow-hidden flex flex-col"
          >
          ...
          <Link
            to="/admin/credits"
            className="group bg-white rounded-3xl p-8 shadow-lg shadow-teal-100/50 border border-teal-50 hover:-translate-y-2 hover:shadow-xl hover:shadow-emerald-200/50 transition-all duration-300 relative overflow-hidden flex flex-col md:col-span-2 lg:col-span-1"
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div className="border-l-2 border-cyan-500/30 pl-4 transition-all hover:border-cyan-500 focus-within:border-cyan-500">
                ...
                <p className="text-3xl font-black text-cyan-400">
                  {loadingStats ? (
                    <span className="loading loading-spinner loading-md text-cyan-500 opacity-50"></span>
                  ) : (
                    stats.presentToday
                  )}
                </p>
              ...
              <div className="border-l-2 border-fuchsia-500/30 pl-4 transition-all hover:border-fuchsia-500 focus-within:border-fuchsia-500">
                ...
                <p className="text-3xl font-black text-white">
                  {loadingStats ? (
                    <span className="loading loading-spinner loading-md text-fuchsia-500 opacity-50"></span>
                  ) : (
                    stats.certsIssued.toLocaleString()
                  )}
                </p>
              </div>
              <div className="border-l-2 border-emerald-500/30 pl-4 transition-all hover:border-emerald-500 focus-within:border-emerald-500">
                ...
                <p className="text-3xl font-black text-white flex items-end gap-1">
                  {loadingStats ? (
                    <span className="loading loading-spinner loading-md text-emerald-500 opacity-50"></span>
                  ) : (
                    <>
          ...
          <div className="md:col-span-2 lg:col-span-3 bg-white rounded-3xl p-8 shadow-lg border border-teal-100">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-slate-800">Gatepass Entry Requests</h3>
              <span className="text-sm font-bold bg-amber-100 text-amber-700 px-3 py-1 rounded-full">
                {pendingEntryRequests.length} Pending
              </span>
            </div>
            {pendingEntryRequests.length === 0 ? (
              <p className="text-slate-500 font-medium">No pending gatepass entry requests.</p>
            ) : (
              <div className="space-y-3">
                {pendingEntryRequests.map((request) => (
                  <div
                    key={request.id}
                    className="border border-slate-100 rounded-xl p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3"
                  >
                    <div className="text-sm text-slate-600 space-y-1">
                      ...
                      <p>
                        Exited: <span className="font-semibold">{new Date(request.exitAt).toLocaleString()}</span>
                      </p>
                      <p>
                        Requested Entry: <span className="font-semibold">{new Date(request.requestedAt).toLocaleString()}</span>
                      </p>
                    </div>
                    <button
                      onClick={() => handleApproveEntryRequest(request.id)}
                    ...
                    >
                      {approvingRequestId === request.id ? 'Approving...' : 'Approve Entry'}
                    </button>
                  </div>
                ))}
              </div>
