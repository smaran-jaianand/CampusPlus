                        ...
                        {(!activeAddress || isAuthorizedAdmin) && navLinks.map((link) => {
                            ...
                            return (
                                <Link
                                    key={link.name}
                                    to={link.path}
                                    className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${isActive ? 'bg-teal-100 text-teal-700 shadow-sm' : 'hover:bg-gray-100 text-gray-600'
                                        }`}
                                >
                                    {link.icon}
                                    {link.name}
